...
func TestImpersonator(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantLogs:       []string{"\"error\"=\"missing bearer token\" \"msg\"=\"invalid token encoding\" \"method\"=\"GET\" \"url\"=\"http://pinniped.dev/blah\""},
		},
		{
		...
		{
			name:           "authorization header has different prefix",
			getKubeconfig:  func() (*rest.Config, error) { return &testServerKubeconfig, nil },
			request:        newRequest(map[string][]string{"Authorization": {"Basic " + makeTestTokenRequest("foo", "authenticator-one", "test-token")}}),
			wantHTTPBody:   "invalid token encoding\n",
			wantHTTPStatus: http.StatusBadRequest,
			wantLogs:       []string{"\"error\"=\"missing bearer token\" \"msg\"=\"invalid token encoding\" \"method\"=\"GET\" \"url\"=\"http://pinniped.dev/blah\""},
		},
		{
			name:           "authorization header missing space after bearer prefix",
			getKubeconfig:  func() (*rest.Config, error) { return &testServerKubeconfig, nil },
		...
		{
			name:           "authorization header has empty bearer token",
			getKubeconfig:  func() (*rest.Config, error) { return &testServerKubeconfig, nil },
